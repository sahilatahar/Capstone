...
data.slice(1, 3).forEach((course) => {
	trendingSection.innerHTML += `<div class="card ${course.title}__Card">
                <div class="complete__mark">${
					userData != null &&
					userData?.completedCourses?.includes(course.title)
						? "Completed"
						: ""
				}</div>
                <img src="./images/${course.poster}" alt="${
		course.title
...
data.forEach((course) => {
	...
	cards.forEach((card) => {
		card.addEventListener("click", () => {
			...
			window.location.href = `/course.html?id=${course.courseId}`
		})
	})
})
