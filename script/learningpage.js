...
// Showing Navbar if screen size > 768;
...
document.querySelectorAll(`.list__item`).forEach((item, itemIndex) => item.addEventListener('click', (e) => {
    index = itemIndex
    changeVideo(index);
}));
    ...
    if (e.target.id === 'next__btn') {
        ...
        if (index < cCourse.videos.length - 1) {
            ...
            if ((progress / cCourse.videos.length) * 100 >= 50) {
                let userData = JSON.parse(localStorage.getItem('userData')) || { completedCourses: [] };
                ...
                document.querySelector('.error__text').textContent = "You should complete at least 50% of the Course";
                return;
            }
        }
    } else {
